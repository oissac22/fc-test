...
export class SericeInsertUser {
    ...
    private verifyData()
    {
        ...
        if (!/\w{2,}/.test(login))
            throw new HTTPException(`"${login}" não é um nome válido`, HTTPStatus.NOT_ACCEPTABLE, 'notValidLogin');
        if (!/\w{6,}/.test(password))
            throw new HTTPException(`"${password}" não é uma senha válido, a senha deve conter 6 ou mais digitos`, HTTPStatus.NOT_ACCEPTABLE, 'notValidPassword');
